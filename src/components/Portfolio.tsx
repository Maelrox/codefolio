...
import { FaLaptopCode, FaDatabase, FaServer, FaMobileAlt, FaCloudflare, FaGamepad, FaBrain } from 'react-icons/fa';

interface PortfolioProps {
  onViewChange: (view: string, tv: string, textureUrl: string) => void;
}
...
const Portfolio: React.FC<PortfolioProps> = ({ onViewChange }) => {
  ...
  const portfolioItems = [
    ...
    { icon: <FaDatabase />, text: 'Devops CI/CD', view: "React", textureUrl: "/assets/3d/cicd_background.jpg", tv:"tv2" },
    ...
    <section className="portfolio p-2">
      <div className="grid grid-cols-2 sm:grid-cols-6 lg:grid-cols-6 gap-4">
        {portfolioItems.map((item, index) => (
          <div key={index} onClick={() => onViewChange(item.view, item.tv, item.textureUrl)}>
