...
const DevRoomModel = ({ tvNumber, textureUrl, setModel, setCameras, updateTvImages }) => {
  
    const gltf = useLoader(GLTFLoader, "/assets/3d/dev-room2.glb");
    const newTexture = useLoader(THREE.TextureLoader, textureUrl);
    console.log("devroom rendered with " + textureUrl + " update " + updateTvImages + " tv number " + tvNumber);
    const emissiveColor = new THREE.Color(0xffffff); // Emissive color
    ...
    const processedModel = useMemo(() => {
      ...
      return gltf.scene;
    }, [gltf.cameras, gltf.scene, newTexture, setCameras, tvNumber, updateTvImages]);
